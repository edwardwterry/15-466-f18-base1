...
void WalkMesh::walk(WalkPoint &wp, glm::vec3 const &step) const {
	...
	// 	//if there is another triangle over the edge:
		...
		switch (crossed_edge){
			case 0:
				v = next_vertex.find(glm::uvec2(wp.triangle.z, wp.triangle.y))->second;
				next_tri_vertices[0] = v;
				std::swap(next_tri_vertices[1], next_tri_vertices[2]);
				break;
			case 1:
				v = next_vertex.find(glm::uvec2(wp.triangle.x, wp.triangle.z))->second;
				next_tri_vertices[1] = v;
				std::swap(next_tri_vertices[0], next_tri_vertices[2]);
				break;
			case 2:
				v = next_vertex.find(glm::uvec2(wp.triangle.y, wp.triangle.x))->second;
				next_tri_vertices[2] = v;
